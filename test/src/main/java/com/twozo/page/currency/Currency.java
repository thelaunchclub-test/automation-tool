...
import com.twozo.extent.report.reporter.ExtentLogger;
...
public class Currency extends BasePage {
    ...
    /**
     * <p>
     * Selects and sets the base currency according to the provided CurrencyData object.
     * </p>
     *
     * @param currencyData Containing information about the currency to be set.
     */
//    public void getBaseCurrency(final CurrencyData currencyData) {
//        final String currency = currencyData.getCurrencyName();
//
//        if (currencyData.isAddCurrency()) {
//            click(getAddCurrencyButton());
//        }
//
//        if (currencyData.isPopUpVisible()) {
//            click(getYes());
//        }
//
//        if (Objects.nonNull(currency)) {
//            click(currencyData.isDefault() ? getBaseCurrencyBox() : getAddCurrencyBox());
//            try {
//                dropdown(currency);
//                Thread.sleep(10000);
...
//            }
//            click(currencyData.isDefault() ? getChange() : getSave());
//        }
//
//        final int decimal = currencyData.getDecimal();
//
//        if (0 != decimal) {
//            click(getDecimalDropDown());
//            dropdown(String.valueOf(decimal));
//            click(getYes());
//        }
//
//        if (currencyData.isListView()) {
//            handleCurrencyEnableDisable(currencyData);
//        }
//    }
    ...
    public boolean isPopUpVisible() {
        click(getAddCurrencyButton());
        ExtentLogger.pass("Add currency button is clicked");
    ...
    /**
     * Handles the enable/disable functionality for a currency based on provided data.
     *
     * @param currencyData The CurrencyData object containing information about the currency's state.
     */
//    private void handleCurrencyEnableDisable(final CurrencyData currencyData) {
//
//        if (currencyData.isEnable()) {
//            try {
//                isDisplayed(getEnableButton());
//                click(getCurrencyButton());
//            } catch (Exception e) {
//                isDisplayed(getDisableButton());
//                click(getCurrencyButton());
...
//    }
    ...
    public void getDecimal(final TestCase testCase) {
        ...
        click(getYes());
        ExtentLogger.pass("Yes button is clicked");
        try {
            Thread.sleep(3000);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
    ...
    public void addCurrency(final TestCase testCase) {
        ...
        if (Objects.nonNull(currency)) {
            ...
            try {
                ...
                ExtentLogger.pass("Add currency field is clicked");
                final Collection<WebPageElement> currencyOptions = findElementsByXpath("//*[@class='MuiList-root MuiList-padding MuiMenu-list css-r8u8y9']");
...
//
//    /**
//     * Disables a currency based on the test case input.
//     *
//     * @param testCase the test case object containing input data, including the currency to be disabled.
//     */
//    public void disAbleCurrency(final TestCase testCase) {
//        final String currency = testCase.input.getString(CURRENCY_TYPE);
//
//        if (testCase.input.getBoolean("isEnable")) {
...
//    /**
//     * Enables a currency based on the test case input.
//     *
//     * @param testCase the test case object containing input data, including the currency to be enabled.
//     */
...
//    }
    ...
    public Collection<String> getAvailableCurrenciesInAddForm() {
        final Collection<WebPageElement> currencyElements = findElementsByXpath("//*[@class='MuiList-root MuiList-padding MuiMenu-list css-r8u8y9']");
    ...
    /**
     * Verifies if a currency can be added by selecting it from the dropdown and saving it.
     *
     * @param testCase the test case object containing the currency to be added.
     */
//    public void verifyAddCurrency(final TestCase testCase) {
//        final String currency = testCase.input.getString(CURRENCY_TYPE);
//
...
//    }
    ...
    /**
     * Selects a currency from the dropdown in the add currency form.
     *
     * @param currency the currency to be selected from the dropdown.
     * @return null after the selection is completed.
     */
//    private String selectCurrencyFromDropdown(final String currency) {
//        try {
//            click(getAddCurrencyBox());
//
//            if (isCurrencyInDropdown(currency)) {
//                dropdown(currency);
//            } else {
//                throw new NoSuchElementException("Currency not found in dropdown: " + currency);
//            }
//        } catch (Exception e) {
//            handleCurrencySelectionException(currency);
//        }
//
//        return null;
//    }
    ...
    /**
     * Handles exceptions during currency selection from the dropdown.
     *
     * @param currency the currency that caused the exception.
     */
    private void handleCurrencySelectionException(final String currency) {
        click(getAddCurrencyBox());
        dropdown(currency);

        try {
            explicitWaitHandler.wait(2000);
        } catch (InterruptedException ie) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("Thread was interrupted while waiting: " + ie.getMessage(), ie);
        }
    }

    /**
     * Checks if a specific currency is present in the dropdown list.
     *
     * @param currency the currency to check in the dropdown list.
     * @return true if the currency is found in the dropdown, false otherwise.
     */
    private boolean isCurrencyInDropdown(final String currency) {
        return !findElementsByXpath("//*[@class='MuiList-root MuiList-padding MuiMenu-list css-r8u8y9'][contains(text(), '" + currency + "')]").isEmpty();
    }
    ...
    public Collection<WebPageElement> getCurrencyList() {
        return findElementsByXpath("//*[@class='MuiPaper-root MuiPaper-elevation MuiPaper-rounded MuiPaper-elevation1 MuiPaper-root MuiMenu-paper MuiPaper-elevation MuiPaper-rounded MuiPaper-elevation8 MuiPopover-paper css-15leva4']");
    }
}